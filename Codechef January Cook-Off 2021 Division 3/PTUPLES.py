...
def primeSeive(n):
	...
	primeList=[]
	#print(seive)
	for idx in range(len(seive)):
		if seive[idx]:
			primeSet.add(idx)
			primeList.append(idx)
	return(primeList,primeSet)

from collections import defaultdict
import bisect
def main():
	primeList,primeSet=primeSeive(pow(10,6)+1)
	prefixSum=0
	ansHash=defaultdict(int)
	for i in primeList:
		if i+2 in primeSet:
			prefixSum+=1
			ansHash[i+2]=prefixSum
	keySorted=sorted(ansHash.keys())
	for _ in range(int(input())):
		x=int(input())
		if x<5:
			print(0)
			continue
		print(ansHash[keySorted[bisect.bisect_left(keySorted, x)]])
