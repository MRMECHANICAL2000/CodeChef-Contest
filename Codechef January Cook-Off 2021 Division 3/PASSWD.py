...
def main():
	for _ in range(int(input())):
		...
		if len(password)<10:
			...
			continue
		condition[4]=True
		if 97<=ord(password[0])<=122: #Lower Case
			condition[0]=True

		if 97<=ord(password[-1])<=122: #Lower Case
			condition[0]=True

		for idx in range(1,len(password)-1):
			val=password[idx]
			if 97<=ord(val)<=122: #Lower Case
				condition[0]=True

			elif 65<=ord(val)<=90: #Upper Case
				condition[1]=True

			elif 48<=ord(val)<=57: #Integer
				condition[2]=True

			elif val in ['@', '#', '%', '&', '?' ]: #Special Character
				condition[3]=True
