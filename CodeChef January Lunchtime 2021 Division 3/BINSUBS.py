...
#Did not solved during the Contest
#Using Linear Dynamic Programming Technique
#Must Watch the vide -> https://youtu.be/nz6y-lD09wk
def main():
    for _ in range(int(input())):
        n=int(input())
        arr=input()
        zeroCount=1 if arr[0]=="0" else 0
        DP=[1]+[0 for i in range(len(arr)-1)]
        for idx in range(1,len(arr)):
        	if arr[idx]=="0":
        		zeroCount+=1
        		DP[idx]=max(DP[idx-1],zeroCount)
